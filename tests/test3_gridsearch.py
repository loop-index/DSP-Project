...
try:
    ...
    categories = pd.concat([merged_data['active_seconds'], categories], axis=1)

    # merged_data = pd.concat([merged_data, categories], axis=1)
    merged_data = merged_data.drop(columns=['category', 'active_seconds'])
    ...
    X_train, X_test, y_train, y_test = train_test_split(categories, merged_data[['age_recode']], test_size=0.01, random_state=42)
