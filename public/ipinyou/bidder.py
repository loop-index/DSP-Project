...
def load_models():
    try:
        ...
        imps = pd.read_csv("public/ipinyou/data/imp.20130612.txt", sep='\t', header=None, usecols=[0, 1, 17, 19, 20, 22, 23])
        imps = imps.dropna()
        imps.columns = ['BidID', 'Timestamp', 'FloorPrice', 'Bid', 'Paid', 'AdvertiserID', 'UserTags']
        imps['Timestamp'] = pd.to_datetime(imps['Timestamp'], format='%y%m%d%H%M%S%f')
        ...
        X_train, X_test, y_train, y_test = train_test_split(data.drop(['Bid'], axis=1), data['Bid'], test_size=0.2, random_state=42)
...
def get_bid(user_tags, floor_price, advertiserID, ctr):
    ...
    input_data = pd.DataFrame({'FloorPrice': floor_price, 'AdvertiserID': advertiserID}, index=[0], dtype='int64')
